...
Or from the CLI via ``python -m audio_pipeline.pipeline.package``.
"""
...
from audio_pipeline.packaging.clips import CUT_TIERS, Clip, build_clips
from audio_pipeline.packaging.loaders import (
    ESCLoader,
    FeatureLoader,
    VADLoader,
    VTCLoader,
...
from audio_pipeline.packaging.writer import write_shards
...
class Packer:
    ...
    def build_clips(self) -> list[tuple[str, Path, int, Clip]]:
        ...
        loaders_by_name: dict[str, FeatureLoader] = {l.name: l for l in self._loaders}
        ...
        for row in self.manifest_df.iter_rows(named=True):
            ...
            for loader in self._loaders:
                file_features[loader.name] = loader.load_file(self.output_dir, uid)
    ...
    def write_manifest(self, shard_dir: Path | None = None) -> Path:
        ...
        """
        from audio_pipeline.core import VTC_LABELS
