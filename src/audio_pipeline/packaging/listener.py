...
Usage:
    python -m audio_pipeline.packaging.listener output/seedlings/shards
    python -m audio_pipeline.packaging.listener output/seedlings/shards -n 50 --diverse --wav
    python -m audio_pipeline.packaging.listener output/seedlings/shards --clip_id uid_0003
"""
...
def extract_clips(
    ...
    if clip_ids:
        ...
        print(
            f"Diverse sampling: {len(selected)} clips across {len(_BUCKETS)} categories"
        )
    else:
        random.seed(seed)
