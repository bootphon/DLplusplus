...
Usage (must run on GPU node):
    uv run python -m audio_pipeline.analysis.snr_vad_diagnostic seedlings_1
"""
...
from audio_pipeline.utils import get_dataset_paths, load_manifest, set_seeds
...
_DEFAULT_MODEL: Path = _MODEL_ROOT / "brouhaha/best.ckpt"
...
def _load_model(model_path: str, device: str):
    ...
    dev = torch.device(device if torch.cuda.is_available() else "cpu")  # pyright: ignore[reportPrivateImportUsage]
    model.to(dev)
...
def main(dataset: str, device: str = "cuda"):
    ...
    pipeline = _load_model(str(_DEFAULT_MODEL), device)
