...
Usage:
    python -m audio_pipeline.pipeline.preflight chunks30
    python -m audio_pipeline.pipeline.preflight chunks30 --sample 500
    eval "$(python -m audio_pipeline.pipeline.preflight chunks30 --emit-env)"
"""
...
def _rates_from_benchmarks() -> tuple[float | None, float | None]:
    ...
    if vtc_records:
        rates = [
            r["bytes_per_second"]
            for r in vtc_records
            if r.get("bytes_per_second", 0) > 0
        ]
        if rates:
            rates.sort()
