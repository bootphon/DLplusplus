...
Usage (typically called automatically from pipeline.sh):
    python -m audio_pipeline.pipeline.normalize my_data --manifest /data/meta.xlsx \\
...
def normalize_manifest(
    ...
    target = PROJECT_PATHS.manifests_dir / f"{dataset}.csv"
    ...
    df.write_csv(target)
    print(f"  Normalized: {target}  ({len(df)} rows, {len(df.columns)} cols)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python -m audio_pipeline.pipeline.normalize my_data --manifest metadata.csv "
