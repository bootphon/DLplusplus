...
Usage:
    python -m audio_pipeline.pipeline.esc chunks30
    python -m audio_pipeline.pipeline.esc chunks30 --pool_window 1.0
...
def main(
    ...
    if not meta_df.is_empty():
        ...
        if not ok.is_empty():
            ...
            logger.info(
                f"  Bins      : {ok['n_pooled_bins'].sum():,} ({pool_window}s windows)"
            )
...
def entrypoint() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python -m audio_pipeline.pipeline.esc chunks30\n"
            "  python -m audio_pipeline.pipeline.esc chunks30 --pool_window 0.5\n"
        ),
    )
    parser.add_argument(
        "dataset",
