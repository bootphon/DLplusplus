...
Used by ``audio_pipeline_p.ppipeline.snr`` and ``audio_pipeline.pipeline.segment_snr`` to avoid
...
def ensure_model(model_path: Path | os.PathLike) -> Path:
...
def load_brouhaha_pipeline(model_path: Path, device: str = "cuda"):
