...
from audio_pipeline.plotting.utils import lazy_pyplot, save_figure
...
def save_master_overview(
    ...
    ax.set_ylabel("Count")
    ax.set_title(
        f"VAD–VTC Agreement (IoU)\n{n_zero} clips with IoU=0 (near-silent)",
        fontsize=10,
    )
...
def save_master_quality(
    ...
    ax = axes[2, 1]
    cat_stats_path = (
        esc_stats_dir / "category_stats.parquet" if esc_stats_dir is not None else None
    )
    _esc_cat_df = None
...
def print_dataset_summary(
    ...
    durs = clip_df["duration"].to_numpy()
    print(
        f"  Clip duration    : median={np.median(durs):.0f}s  "
    ...
    for l in VTC_LABELS:
        ...
        if col in clip_df.columns:
            ...
            n_seg = (
                int(segment_df.filter(pl.col("label") == l).height)
                if len(segment_df) > 0
                else 0
            )
    ...
    if len(snr_vals) > 0:
        print(
            f"  SNR  (n={len(snr_vals):,}) : median={np.median(snr_vals):.1f} dB  "
        ...
        if "snr_during" in segment_df.columns:
            ...
            if len(seg_snr) > 0:
                ...
                for l in VTC_LABELS:
                    ...
                    if len(v) > 0:
                        print(
                            f"    {l:6s} : mean={np.mean(v):.1f} dB  std={np.std(v):.1f}"
                        )
    if len(c50_vals) > 0:
        print(
            f"  C50  (n={len(c50_vals):,}) : median={np.median(c50_vals):.1f} dB  "
    ...
    if n_turns > 0:
        ...
        print(
            f"  Turn duration    : median={np.median(td):.2f}s  "
        ...
        if len(td_per_min) > 0:
            print(
                f"  Turn density     : median={np.median(td_per_min):.1f}/min  "
    ...
    if n_convs > 0:
        ...
        print(
            f"  Single-speaker   : {n_convs - multi:,} ({100 * (n_convs - multi) / n_convs:.1f}%)"
        )
        ic_gaps = conv_df["gap_after"].drop_nulls().to_numpy()
        if len(ic_gaps) > 0:
            print(
                f"  Inter-conv gap   : median={np.median(ic_gaps):.1f}s  "
        ...
        if "snr_mean" in conv_df.columns:
            ...
            if len(cs) > 0:
                print(
                    f"  Conv SNR         : median={np.median(cs):.1f} dB  "
        ...
        if "c50_mean" in conv_df.columns:
            ...
            if len(cc) > 0:
                print(
                    f"  Conv C50         : median={np.median(cc):.1f} dB  "
