...
from dataloader.paths import PROJECT_PATHS
...
def _stage_done(output_dir: Path, stage: str) -> bool:
    ...
    if stage == "vad":
        return (output_dir / "vad_meta" / "metadata.parquet").exists() and (
            output_dir / "vad_merged" / "segments.parquet"
        ).exists()
    if stage == "vtc":
        return bool(list((output_dir / "vtc_meta").glob("shard_*.parquet"))) and bool(
            list((output_dir / "vtc_merged").glob("shard_*.parquet"))
        )
    if stage == "snr":
...
def _check_jobs_ok(job_ids: list[str], label: str) -> None:
    ...
    if failed:
        ...
        raise RuntimeError(f"{label} jobs did not complete successfully: {details}")
...
def prepare(
    ...
    *,
    ...
    try:
        preflight_result = subprocess.run(
            [
                ...
                "python",
                "-m",
                "audio_pipeline.pipeline.preflight",
                dataset,
                "--emit-env",
            ]
            + extra_args,
    ...
    logs_dir = PROJECT_PATHS.logs_dir / "logs"
    ...
    if run_vad:
        ...
        print("[prepare]   VAD              : already done, skipping")
    ...
    if run_vtc:
        ...
        print("[prepare]   VTC              : already done, skipping")

    if run_snr:
        snr_jid = _sbatch(
            [
                "--array",
                f"0-{snr_array_count - 1}",
                str(slurm_dir / "snr.slurm"),
                dataset,
            ]
            + extra_args
        )
        ...
        print("[prepare]   SNR              : already done, skipping")

    if run_esc:
        esc_jid = _sbatch(
            [
                "--array",
                f"0-{esc_array_count - 1}",
                str(slurm_dir / "esc.slurm"),
                dataset,
            ]
            + extra_args
        )
        ...
        print("[prepare]   ESC              : already done, skipping")
    ...
    pkg_cmd = (
        ...
        f"PYTHONUNBUFFERED=1 uv run python -m audio_pipeline.pipeline.package {dataset}"
        f" --audio_fmt {p.audio_fmt}"
        f" --max_clip {p.max_clip_s}"
        f" --split_search {p.split_search_s}"
        f" --shard_size {p.shard_size}"
        f" --target_sr {p.target_sr}" + (f" --sample {sample}" if sample else "")
    )
    ...
    pkg_sbatch_args = [
        ...
        "dlpp_package",
        "--output",
        str(logs_dir / "package" / "pkg_%j.out"),
        "--error",
        str(logs_dir / "package" / "pkg_%j.err"),
        "--cpus-per-task",
        "8",
        "--mem",
        "64G",
